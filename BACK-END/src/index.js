    ...
    file.mv(__dirname + '/public/uploads/'+ file.name, async (err, result) => { 
        if(err){
            ...
            if(title.length > 0 & body.length > 0 & file.name.length > 0){ 
                ...
                req.flash('message', '¡Rellena todos los campos!')
    ...
    if(req.files){
        ...
        file.mv(__dirname + '/public/uploads/'+ file.name, async (err, result) => { 
            if(err){
                ...
                req.flash('success', '¡Post editado con éxito!')
                res.redirect('/posts')
            }
        })
    }else {
...
app.get('/posts/to_delete/:id', isAuthenticatedAdmin, async (req, res) => {
    ...
    req.flash('success', '¡Post movido a la papelera!')
    res.redirect('/posts')
})
...
app.get('/posts/revive/:id', isAuthenticatedAdmin, async (req, res) => {
    ...
    req.flash('success', '¡Post resucitado con éxito!')
    res.redirect('/posts')
})
...
app.get('/posts/delete_all', isAuthenticatedAdmin, async (req, res) => {
    ...
    req.flash('success', '¡Posts eliminados con éxito!')
    ...
    if(req.files){
        ...
        file.mv(__dirname + '/public/uploads/'+ file.name, async (err, result) => { 
            if(err){
                ...
                req.flash('success', '¡Profile editado con éxito!')
